...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        state_nodes = set()
        #Action nodes of one level down has to be connected to current state
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for pos_effect_a in node_a1.action.effect_add:
            if pos_effect_a in node_a2.action.effect_rem:
                return True

        for pos_effect_b in node_a2.action.effect_add:
            if pos_effect_b in node_a1.action.effect_rem:
                return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for pos_effect_a in node_a1.action.effect_add:
            if pos_effect_a in node_a2.action.precond_neg:
                return True

        for pos_effect_b in node_a2.action.effect_add:
            if pos_effect_b in node_a1.action.precond_neg:
                return True

        for neg_effect_a in node_a1.action.effect_rem:
            if neg_effect_a in node_a2.action.precond_pos:
                return True

        for neg_effect_b in node_a2.action.effect_rem:
            if neg_effect_b in node_a1.action.precond_pos:
                return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for a_node_a1 in node_s1.parents:
            for a_node_a2 in node_s2.parents:
                if not a_node_a1.is_mutex(a_node_a2):
                    return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        #https://discussions.udacity.com/t/understand-level-sum-heuristic/225706
